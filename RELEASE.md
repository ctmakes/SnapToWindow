...
```bash
npx @tauri-apps/cli signer generate -w ~/.tauri/snaptowindow.key
```

This creates:
- `~/.tauri/snaptowindow.key` - Private key (keep secret, never commit)
...
The bump script handles everything: version updates, commit, tag, and push. The workflow runs automatically. Check **Actions** tab for progress.
...
| `TAURI_SIGNING_PRIVATE_KEY` | Contents of `~/.tauri/snaptowindow.key` |
